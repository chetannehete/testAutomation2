...
def _infer_filename(lang: str, ext: str, code: str, step: int, seen: dict) -> str:
    """Try to produce a meaningful filename from code content."""
    ...
    if lang in ("plantuml", "puml"):
        ...
        if m:
            return f"{prefix}{m.group(1)}{ext}"
        return f"{prefix}c4_diagram{ext}"

    if lang in ("bash", "shell", "sh"):
        return f"{prefix}run_commands{ext}"

    if lang in ("markdown", "md"):
        return f"{prefix}documentation{ext}"

    if lang in ("yaml", "yml"):
        return f"{prefix}config{ext}"
...
_PROVIDER_DEFAULTS = {
    "gemini":    ("gemini-1.5-pro",   "GEMINI_API_KEY",    _call_gemini),
