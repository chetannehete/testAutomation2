...
Controls the execution flow of the nested prompt generation system:
  ...
  5. Generates all applicable prompts via prompt_templates
...
class PromptOrchestrator:
    ...
    Analyzes a Spring Boot codebase and generates targeted prompts
    based on detected patterns and conditional rules.
    """
    ...
    def plan_execution(self) -> OrchestrationPlan:
        ...
        # ── Condition: API integration tests ──
        conditions["has_rest_controllers"] = features.has_rest_controllers
        if features.has_rest_controllers:
            prompts_to_gen.append(PromptType.INTEGRATION_TEST)

        # ── Condition: DB integration tests ──
        conditions["has_database"] = features.has_database
        # (already covered by INTEGRATION_TEST if not added)

        # ── Condition: Messaging integration tests ──
    ...
    def generate_prompts(self) -> List[GeneratedPrompt]:
        """Step 3: Generate all child prompts according to the orchestration plan."""
        ...
        for prompt_type in self.plan.execution_order:
            if prompt_type == PromptType.UNIT_TEST:
                self._generate_unit_test_prompts()
            elif prompt_type == PromptType.INTEGRATION_TEST:
                self._generate_integration_test_prompts()
    ...
    def generate_master_prompt(self) -> GeneratedPrompt:
        """Step 3b: Generate the master/root orchestrator prompt that chains all child prompts."""
        if not self.generated_prompts or not self.analysis:
            raise RuntimeError("Call generate_prompts() before generate_master_prompt()")

        # Pre-compute the child filenames (same logic as write_output)
        child_filenames = [
    ...
    def write_output(self, output_dir: str) -> Dict[str, str]:
        ...
        Output structure:
            00_master_prompt.md          ← Root prompt that invokes all children
            00_orchestration_report.md   ← Feature detection summary
            01_<type>_prompt.md          ← Child prompt 1
            02_<type>_prompt.md          ← Child prompt 2
            ...
        ...
        # Pre-compute child filenames so the master can reference them
        child_filenames = [
            f"{i:02d}_{p.prompt_type.value}_prompt.md"
            for i, p in enumerate(self.generated_prompts, start=1)
        ]
        ...
        # Write each child prompt as a numbered .md file with 'Invoked By' header
    ...
    # ── Internal prompt generators ──────────────────────────────────────

    def _generate_unit_test_prompts(self):
        """Generate unit test prompts for each testable component."""
        for comp in self.analysis.components:
            if comp.component_type in (ComponentType.TEST, ComponentType.UNKNOWN,
                                        ComponentType.DTO, ComponentType.ENTITY):
                continue
            if not comp.class_info.methods:
                continue
            prompt = build_unit_test_prompt(comp, self.analysis)
            self.generated_prompts.append(prompt)

    def _generate_integration_test_prompts(self):
        """Generate integration test prompts based on detected patterns."""
        features = self.analysis.features
        ...
        if features.has_rest_controllers:
            for comp in self.analysis.components:
                if comp.component_type in (ComponentType.REST_CONTROLLER, ComponentType.CONTROLLER):
                    prompt = build_integration_test_prompt(comp, self.analysis, test_type="api")
                    self.generated_prompts.append(prompt)

        # DB integration tests for repositories
        if features.has_database:
            for comp in self.analysis.components:
                if comp.component_type == ComponentType.REPOSITORY:
                    prompt = build_integration_test_prompt(comp, self.analysis, test_type="db")
                    self.generated_prompts.append(prompt)

        # Messaging integration tests
        if features.has_messaging:
            for comp in self.analysis.components:
                if comp.component_type in (ComponentType.MESSAGING_LISTENER, ComponentType.MESSAGING_PRODUCER):
                    prompt = build_integration_test_prompt(comp, self.analysis, test_type="messaging")
                    self.generated_prompts.append(prompt)
    ...
    def _build_report(self) -> str:
        """Build the orchestration report markdown."""
        features = self.analysis.features
        plan = self.plan
        ...
        for i, p in enumerate(self.generated_prompts, 1):
            prompt_rows.append(
                f"| {i} | {p.prompt_type.value} | {p.purpose} | {', '.join(p.target_classes[:3])}{'...' if len(p.target_classes) > 3 else ''} |"
            )
        prompt_table = "\n".join(prompt_rows)
...
| Feature | Status |
|---------|--------|
| REST Controllers | {'✅' if features.has_rest_controllers else '❌'} ({features.controller_count}) |
| Database / JPA | {'✅' if features.has_database else '❌'} ({', '.join(features.database_types) if features.database_types else '-'}) |
| Messaging | {'✅ Kafka' if features.has_kafka else '✅ RabbitMQ' if features.has_rabbitmq else '❌'} |
| Security | {'✅' if features.has_security else '❌'} |
| Scheduling | {'✅' if features.has_scheduling else '❌'} |
| Caching | {'✅' if features.has_caching else '❌'} |
| Validation | {'✅' if features.has_validation else '❌'} |
| Feign Client | {'✅' if features.has_feign_client else '❌'} |
| Circuit Breaker | {'✅' if features.has_circuit_breaker else '❌'} |
| Exception Handling | {'✅' if features.has_exception_handling else '❌'} |
...
"""
    ...
    def _format_prompt_file(self, prompt: GeneratedPrompt, step_number: int = 0, total_steps: int = 0) -> str:
        ...
        # Build the 'Invoked By' chain context header
        invoked_by_header = ""
        if step_number > 0:
