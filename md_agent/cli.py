...
def orchestrate(path, output_dir, project_name, recursive):
    ...
    if not java_files:
        click.secho(f"⚠  No .java files found at: {path}", fg="yellow")
        sys.exit(1)
    ...
    if not all_classes:
        click.secho("⚠  No Java classes found after parsing.", fg="yellow")
        sys.exit(1)
    ...
    click.echo(f"   Database:    {'✅ ' + ', '.join(features.database_types) if features.has_database else '❌'}")
    click.echo(f"   Messaging:   {'✅ Kafka' if features.has_kafka else '✅ RabbitMQ' if features.has_rabbitmq else '❌'}")
    click.echo(f"   Security:    {'✅' if features.has_security else '❌'}")
    ...
    for cond, met in plan.conditions_met.items():
        status = click.style("✅", fg="green") if met else click.style("❌", fg="red")
    ...
    click.echo(f"\n{'═' * 50}")
    click.secho(
        f"✅ Done! Generated {len(prompts)} prompt(s) + orchestration report → {output_dir}/",
...
@click.option("--model", "-m", default=None,
              help="Model name (default: gemini-1.5-pro / gpt-4o / claude-3-5-sonnet)")
...
def execute(prompts_dir, provider, model, api_key, generated_dir, step, dry_run, delay):
    ...
    def on_step_done(result):
        if result.success:
            if result.extracted_files:
                ...
                click.secho(f"   ⚠  No code blocks extracted", fg="yellow")
            click.secho(f"   ✅ Step {result.step_number:02d} complete", fg="bright_green")
        else:
            click.secho(f"   ❌ Step {result.step_number:02d} failed: {result.error}", fg="red")
    ...
    try:
        ...
        click.secho(f"❌ Session failed: {e}", fg="red")
        sys.exit(1)
    ...
    click.echo(f"\n{'═' * 50}")
    click.secho(
        f"✅ Session complete! {ok}/{total} steps succeeded, "
        f"{artifacts} artifact(s) saved → {generated_dir}/",
        fg="bright_green", bold=True,
    )
    if failed:
        click.secho(f"⚠  {failed} step(s) failed — check session_log.md", fg="yellow")
    ...
    for s in session.steps:
        ...
        purpose_short = s.purpose[:48] + "…" if len(s.purpose) > 48 else s.purpose
        click.echo(f"  {s.step_number:02d}     {status}       {files:<6} {purpose_short}")
    click.echo("  " + "─" * 60)
...
def _common_options(fn):
    ...
    fn = click.option("--output-dir", "-o", default="output",
                      help="Directory to write generated .md files (default: ./output)")(fn)
    fn = click.option("--recursive/--no-recursive", "-r/-R", default=True,
                      help="Recursively scan directories for .java files")(fn)
    fn = click.option("--template-dir", "-t", default=None,
                      help="Custom Jinja2 template directory (overrides built-in templates)")(fn)
    return fn


@cli.command()
@_common_options
def generate(path, output_dir, recursive, template_dir):
    """Generate BOTH test cases and documentation (legacy mode)."""
    _run(path, output_dir, recursive, template_dir, do_tests=True, do_docs=True)


@cli.command()
@_common_options
def testcases(path, output_dir, recursive, template_dir):
    """Generate test case Markdown files only (legacy mode)."""
    _run(path, output_dir, recursive, template_dir, do_tests=True, do_docs=False)


@cli.command()
@_common_options
def docs(path, output_dir, recursive, template_dir):
    """Generate documentation Markdown files only (legacy mode)."""
    _run(path, output_dir, recursive, template_dir, do_tests=False, do_docs=True)


def _run(path, output_dir, recursive, template_dir, do_tests, do_docs):
    java_files = discover_java_files(path, recursive=recursive)
    if not java_files:
        click.secho(f"⚠  No .java files found at: {path}", fg="yellow")
        sys.exit(1)
    ...
    for jf in java_files:
        ...
        for cls in classes:
            click.echo(f"   📦 Class: {cls.name} ({len(cls.methods)} methods, {len(cls.fields)} fields)")
            if do_tests:
                suite = generate_test_suite(cls)
                out_path = render_test_cases(suite, output_dir, template_dir)
                click.secho(f"   ✓ Test cases ({len(suite.test_cases)} TCs) → {Path(out_path).name}", fg="green")
                total_tests += len(suite.test_cases)
            if do_docs:
                doc = generate_documentation(cls)
                out_path = render_documentation(doc, output_dir, template_dir)
                click.secho(f"   ✓ Documentation ({len(doc.sections)} sections) → {Path(out_path).name}", fg="green")
                total_docs += len(doc.sections)

    click.echo(f"\n{'═' * 50}")
    summary_parts = []
    if do_tests:
        summary_parts.append(f"{total_tests} test cases")
    if do_docs:
        summary_parts.append(f"{total_docs} doc sections")
    click.secho(f"✅ Done! Generated {' & '.join(summary_parts)} → {output_dir}/", fg="bright_green", bold=True)
